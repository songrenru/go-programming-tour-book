...
func (d *Dao) CountArticleByTagId(id uint32, state uint8) (int, error) {
	article := model.Article{State: state}
	return article.CountByTagId(d.engine, id)
}

func (d *Dao) GetArticleListByTagId(id uint32, state uint8, page int, size int) ([]*model.ArticleRow, error) {
	article := model.Article{State: state}
	return article.ListByTagId(d.engine, id, app.GetPageOffset(page, size), size)
}

func (d *Dao) GetArticle(id uint32, state uint8) (model.Article, error) {
	article := model.Article{State: state, Model: &model.Model{ID: id}}
